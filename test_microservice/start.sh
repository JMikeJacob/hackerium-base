...
then
  ...
  node node_modules/db-migrate/bin/db-migrate db:create hackerium_test --config ./database/database.json -e default_for_create_database
  ...
  node node_modules/db-migrate/bin/db-migrate db:create test_kvs --config ./database/database.json -e create_key_value_store
  ...
  node node_modules/db-migrate/bin/db-migrate db:create test_eventstore --config ./database/database.json -e create_event_store
  ...
  node node_modules/db-migrate/bin/db-migrate db:create test_write --config ./database/database.json -e create_write_repository
