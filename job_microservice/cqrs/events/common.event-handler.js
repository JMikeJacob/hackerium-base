...
const commonEventHandler = (broker, readRepository, commonCommandHandler) => {
    ...
    this.eventQueue = async.queue(async function(event, callback) {
        ...
        if(event.payload.eventType === CONSTANTS.EVENTS.APPLICATION_TEST_CODE_SUBMITTED) {
            broker.publish(CONSTANTS.TOPICS.ENGINE_REQUEST_TOPIC, {
                payload: {
                    appId: event.payload.appId,
                    script: event.payload.script,
                    testId: event.payload.testId,
                    testParameters: event.payload.testParameters,
                    testFileUrls: event.payload.testFileUrls,
                    testExecutionTime: event.payload.testExecutionTime,
                    action: CONSTANTS.ENGINE.ACTIONS.REQUEST_MICROSERVICE_TESTS
                }
            }, event.payload.appId);
        }
